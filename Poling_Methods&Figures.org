...
RNA-seq analysis was performed using Strand NGS 2.9 software (Strand Life Sciences). Count tables generated in Galaxy were processed and normalized using the DESeq2 package within the Strand NGS 2.9 R console. ANOVA and moderated t-Test were used to performed statistical analysis amongst samples and between groups. Functional enrichment analysis was performed in the ToppGene suite (https://toppgene.cchmc.org). Plots were generated using ggplot2 (v2.2.1) and GOplot (v1.02) packages in Rstudio (v1.0.14).
